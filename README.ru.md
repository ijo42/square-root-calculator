...
1. Появится диалог с кнопками **Скачать** и **Пропустить**
2. Нажмите **Скачать**, чтобы открыть страницу релизов в браузере
3. Нажмите **Пропустить**, чтобы закрыть уведомление и продолжить использовать текущую версию

Для ручной проверки обновлений: **Справка → Проверить обновления**
...
Приложение поддерживает загрузку пользовательских переводов для добавления новых языков или настройки существующих.

### Добавление пользовательских переводов

1. Создайте папку `translations` в директории приложения
2. Создайте JSON файл с кодом языка (например, `de.json` для немецкого)
3. Добавьте переводы в формате JSON
4. Используйте **Язык → Обновить переводы** для загрузки новых переводов

### Пример: Немецкий перевод (`translations/de.json`)
...
Полный список ключей переводов и подробные инструкции см. в [translations/README.md](translations/README.md).

### Альтернативное расположение

Пользовательские переводы также могут быть размещены в:
- `~/.square_root_calculator/translations/` (для конкретного пользователя)

## Документация

Для более подробной информации:
- [Примеры использования](docs/USAGE_EXAMPLES.ru.md) - Подробные сценарии использования и советы
- [Руководство по переводам](translations/README.md) - Как добавить или настроить переводы
